...
}`}function H(e="Header Analyzer",t=""){return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
${t}
<style>${A()}</style>
</head>`}function p(e,t){return H(e,t)+"<body>"}function l(){return"</body></html>"}function c(e=0){return`<div class="nav">
  <a href="/">HOME</a>
  <a href="/history">HISTORY${e>0?` (${e})`:""}</a>
  <a href="/bots">BOTS</a>
  <a href="/stats">STATS</a>
  <a href="/download">DOWNLOAD</a>
</div>`}var n={ITEMS_PER_PAGE:50,MAX_HISTORY:500,SUSPICIOUS_THRESHOLD:50,PROBABLE_BOT_THRESHOLD:70,LABELS:{BOT:"BOT",MAYBE_BOT:"MAYBE BOT",HUMAN:"HUMAN",SUSPICIOUS:"SUSPICIOUS"},CLASSES:{BOT:"bot-label",MAYBE_BOT:"maybe-bot-label",HUMAN:"human-label",REQUEST:"request-full",NAV:"nav",SUB:"sub"},ROUTES:{HOME:"/",HISTORY:"/history",BOTS:"/bots",STATS:"/stats",DOWNLOAD:"/download",REQUEST:"/request/",API_JS:"/api/update-js/"},BOT_CATEGORIES:{AI:"AI",SEARCH:"Search",SOCIAL:"Social",SEO:"SEO",MONITORING:"Monitoring",DEVELOPER:"Developer"}};function G(e){return e.isBot?{text:n.LABELS.BOT,class:n.CLASSES.BOT}:e.suspiciousScore>=n.PROBABLE_BOT_THRESHOLD?{text:n.LABELS.BOT,class:n.CLASSES.BOT}:e.suspiciousScore>=n.SUSPICIOUS_THRESHOLD?{text:n.LABELS.MAYBE_BOT,class:n.CLASSES.MAYBE_BOT}:{text:n.LABELS.HUMAN,class:n.CLASSES.HUMAN}}function u(e){let t=G(e);return`<span class="${t.class}">${t.text}</span>`}function g(e,t){return t&&Object.keys(t).length>0?j(t):`<div class="${n.CLASSES.REQUEST}">
...
${W(e)}`}function j(e){let t=Object.keys(e).length,a=Object.values(e).filter(r=>r).length,o="";for(let[r,i]of Object.entries(e))o+=`<tr><td>${r}</td><td>${i?"[YES]":"[NO]"}</td></tr>`;return`<div class="${n.CLASSES.REQUEST}">
<h2>JAVASCRIPT DETECTION - ${t} FEATURES</h2>
<div>Detected: ${a}/${t} features</div>
<table><thead><tr><th>FEATURE</th><th>SUPPORTED</th></tr></thead>
<tbody>${o}</tbody></table>
</div>`}function W(e){return`<script>
(function(){
const f={};
// Core JS - 10
...
if('${e}'!=='null')fetch('${n.ROUTES.API_JS}${e}',{method:'POST',body:JSON.stringify(f)});
})();
<\/script>`}function h(e){let t=e.bot,a=e.headers.signature&&e.headers["signature-agent"];return`
  <div class="${n.CLASSES.REQUEST}">
    <h3>BOT DETECTION RESULT</h3>
    <div class="${n.CLASSES.SUB}">
  ...
  </div>`}function Y(e){return!e.isBot&&!e.probableBot?e.suspiciousScore===0?"<div><strong>WHY HUMAN:</strong> All expected browser headers present, normal behavior</div>":e.suspiciousScore<30?"<div><strong>WHY HUMAN:</strong> Mostly normal browser behavior</div>":`<div><strong>MINOR ISSUES:</strong>
  ...
  </div>`}function _(){return`<div class="${n.CLASSES.BOT}"><strong>AI SIGNATURE DETECTED</strong></div>`}function b(e,t){return`<div class="request-full ${t?"current":""}">
    ${J(e)}${z(e)}${F(e)}${Q(e)}
  </div>`}function J(e){return`<div class="sub">
    <h4>REQUEST INFO</h4>
    <div class="sub">
  ...
  </div>`}function z(e){return`<h4>HEADERS (${e.headers?Object.keys(e.headers).length:0})</h4>
  <div class="sub">
    ${e.headers?Object.entries(e.headers).map(([a,o])=>`<div><span class="key">${a}:</span> <span class="value">${o}</span></div>`).join(""):"No headers"}
  </div>`}function F(e){return`<h4>NETWORK</h4>
  <div class="sub">
  ...
  <div class="sub">
  ...
  </div>`}function E(e,t){return p("Header Analyzer - FULL DATA")+"<h1>HEADER ANALYZER - CURRENT REQUEST ANALYSIS</h1>"+c(t.length)+V(e)+h(e)+b(e,!0)+g(e.id,e.jsData)+l()}function V(e){return`
    <h2><a href="${n.ROUTES.REQUEST}${e.id}">REQUEST ID: ${e.id}</a></h2>
    <div>
      <span>${e.timestamp}</span>
      <span>${e.network.ip}</span>
      ${u(e.bot)}
      <strong><a href="${n.ROUTES.REQUEST}${e.id}">[ VIEW FULL DETAILS ]</a></strong>
    </div>`}function O(e){let t=e.headers["user-agent"]||"No User Agent",a=X(e,t),o=K(e.timestamp),r=e.geo?`${e.geo.city||"?"}, ${e.geo.country||"?"}`:"";return`<div class="${n.CLASSES.REQUEST}">
    <div>
      <a href="${n.ROUTES.REQUEST}${e.id}"><strong>${a}</strong></a>
      ${u(e.bot)}
    </div>
    <div>
      <strong>${o}</strong> | IP: ${e.network.ip}${r?` | ${r}`:""}
    </div>
    <div style="font-size: 14px !important; color: #666 !important;">
      ${t.slice(0,100)}${t.length>100?"...":""}
    </div>
  </div>`}function X(e,t){if(e.bot.isBot&&e.bot.operator)return`${e.bot.operator} ${e.bot.botName||"Bot"}`;if(e.bot.probableBot&&e.bot.operator)return`${e.bot.operator} (Maybe Bot)`;let a=t.includes("Chrome")?"Chrome":t.includes("Firefox")?"Firefox":t.includes("Safari")?"Safari":"Browser",o=t.includes("Windows")?"Windows":t.includes("Mac")?"macOS":t.includes("Linux")?"Linux":"OS";return`${a} on ${o}`}function K(e){let t=new Date(e),o=Math.floor((new Date-t)/6e4);return o<1?"Just now":o<60?`${o} min ago`:o<1440?`${Math.floor(o/60)} hours ago`:t.toLocaleDateString()+" "+t.toLocaleTimeString()}function Z(e,t=1){let a=e,o=Math.ceil(a/n.ITEMS_PER_PAGE),r=Math.min(Math.max(1,t),o||1),i=(r-1)*n.ITEMS_PER_PAGE,s=i+n.ITEMS_PER_PAGE;return{total:a,totalPages:o,currentPage:r,startIndex:i,endIndex:s,hasPrev:r>1,hasNext:r<o}}function v(e,t,a="items"){if(e.totalPages<=1)return"";let o=[];return o.push('<div class="nav">'),e.hasPrev&&o.push(`<a href="${t}?page=${e.currentPage-1}">\u2190 PREV</a>`),o.push(`<span>Page ${e.currentPage} of ${e.totalPages} (${e.total} ${a})</span>`),e.hasNext&&o.push(`<a href="${t}?page=${e.currentPage+1}">NEXT \u2192</a>`),o.push("</div>"),o.join("")}function R(e,t=1){let a=Z(e.length,t);return{items:e.slice(a.startIndex,a.endIndex),pagination:a}}function S(e,t=1,a="",o,r){let i=r?e.filter(r):a?e.filter($=>JSON.stringify($).toLowerCase().includes(a.toLowerCase())):e,{items:s,pagination:d}=R(i,t),f=r?n.ROUTES.BOTS:n.ROUTES.HISTORY;return p(o)+`<h1>${o} - ${d.total} ITEMS</h1>`+c()+q(a,!r)+v(d,f,"items")+ee(s)+v(d,f,"items")+l()}function q(e,t){return t?`<div class="nav">
    <form method="GET" action="${n.ROUTES.HISTORY}" style="display:inline;">
      <input name="filter" value="${e}" placeholder="Search...">
      <button type="submit">SEARCH</button>
    </form>
  </div>`:""}function ee(e){return e.length===0?'<div class="request-full">No items found</div>':e.map(t=>O(t)).join("")}function x(e,t=1,a=""){return S(e,t,a,"REQUEST HISTORY",null)}function I(e){let t=te(e);return p("Statistics - Header Analyzer")+"<h1>STATISTICS</h1>"+c()+oe(t)+l()}function te(e){let t=e.length,a=e.filter(s=>s.bot.isBot).length,o=e.filter(s=>!s.bot.isBot&&s.bot.suspiciousScore>=n.SUSPICIOUS_THRESHOLD).length,r=t-a-o,i=new Set(e.map(s=>s.network.ip)).size;return{total:t,bots:a,humans:r,maybeBots:o,uniqueIPs:i,botPercent:t>0?Math.round(a/t*100):0,humanPercent:t>0?Math.round(r/t*100):0}}function oe(e){return`
    <div class="stats">
      <h2>Overview</h2>
      <table>
    ...
    </div>`}function N(e,t=1){return S(e,t,"","BOT DETECTION",a=>a.bot.isBot||a.bot.probableBot||a.bot.suspiciousScore>=n.SUSPICIOUS_THRESHOLD)}function B(e,t){let a=ne(e);return p("Download Data - Header Analyzer")+"<h1>DOWNLOAD DATA</h1>"+c()+re(a,t)+l()}function ne(e){return{total:e.length,bots:e.filter(t=>t.bot.isBot).length,humans:e.filter(t=>!t.bot.isBot&&t.bot.suspiciousScore<n.SUSPICIOUS_THRESHOLD).length}}function re(e,t){return`
    <div class="stats">
      ...
      <ul>
        <li><a href="${n.ROUTES.DOWNLOAD}/json" download>Download as JSON</a></li>
        <li><a href="${n.ROUTES.DOWNLOAD}/csv" download>Download as CSV</a></li>
        <li><a href="${n.ROUTES.DOWNLOAD}/zip" download>Download as ZIP</a></li>
      </ul>

      <h3>Data Preview</h3>
      <pre>${JSON.stringify({currentRequest:t.id,totalRequests:e.total,latestTimestamp:t.timestamp},null,2)}</pre>
    </div>`}function P(){return`<div class="${n.CLASSES.REQUEST}">
  ...
  </div>`}function k(e){let t=e.headers["user-agent"]||"None",a=e.headers.referer||e.headers.referrer||"Direct";return`<div class="${n.CLASSES.REQUEST}">
...
</div>`}function C(e){return p(`Request ${e.id} - FULL ANALYSIS`)+"<h1>REQUEST DETAILS - COMPREHENSIVE ANALYSIS</h1>"+c()+se(e)+h(e)+b(e,!0)+de(e)+k(e)+pe(e)+l()}function se(e){return`
    <h2>REQUEST ID: ${e.id}</h2>
    <div>
      <span>${e.timestamp}</span>
      <span>${e.network.ip}</span>
      ${u(e.bot)}
    </div>`}function de(e){return e.jsData&&Object.keys(e.jsData).length>0?g(e.id,e.jsData):P()}function pe(e){return`<h3>Raw JSON Data</h3><pre>${JSON.stringify(e,null,2)}</pre>`}var Pt={async fetch(e,t){let a=new URL(e.url),o=a.pathname;if(o.startsWith(n.ROUTES.API_JS)){let s=o.split("/").pop(),d=await e.json();return await t.HEADER_HISTORY.put(`js_${s}`,JSON.stringify(d)),ce({success:!0})}let r=await le(t);if(o.startsWith(n.ROUTES.REQUEST)){let s=o.split("/")[2],d=r.find(f=>f.id===s);if(d)return d.jsData=await t.HEADER_HISTORY.get(`js_${s}`,{type:"json"}),m(C(d))}if(o===n.ROUTES.STATS)return m(I(r));if(o===n.ROUTES.BOTS)return m(N(r,D(a)));if(o===n.ROUTES.HISTORY)return m(x(r,D(a),a.searchParams.get("filter")||""));if(o===n.ROUTES.DOWNLOAD)return m(B(r,y(e,t)));let i=y(e,t);return i.jsData=await t.HEADER_HISTORY.get(`js_${i.id}`,{type:"json"}),r.unshift(i),r.length>n.MAX_HISTORY&&(r.length=n.MAX_HISTORY),await t.HEADER_HISTORY.put("history",JSON.stringify(r)),m(E(i,r))}};async function le(e){try{return await e.HEADER_HISTORY.get("history",{type:"json"})||[]}catch{return[]}}function D(e){return parseInt(e.searchParams.get("page")||"1")}function m(e){return new Response(e,{headers:{"Content-Type":"text/html;charset=UTF-8"}})}function ce(e){return new Response(JSON.stringify(e),{headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}})}export{Pt as default};
